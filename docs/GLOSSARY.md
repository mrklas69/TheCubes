...
- **Atmospheric fog** *(sez. 31)* — `THREE.Fog(SKY_COLOR, 30, 120)` lineární mlha matchne `scene.background`. Vzdálené objekty plynule blednou. Drží se reference `sceneFog` pro toggle restore. Toggle přes `#settings`.
