...
- **`RESOURCE_YIELD`** — per-KIND fixed yield tabulka v `src/composites/builders.js`. Object `{ resource: count }` per KIND. **Chopable** (yield > 0, per sez. 53 user kalibrace **1 voxel napříč**): spruce/oak/palm/stump/log/cactus = 1 wood, rock = 1 stone. **No-yield**: bush/flower/grass_tuft (= vegetace, hover infotip stačí). Predicate `hasChopYield(kind)` pro UI rozlišení. Fixed per KIND (= deterministic, KISS); random yield per chop je sub-prah (mission-driven extraction → IDEAS). Akumulace je gameplay loop — multi-chop kolem column = bucket plní postupně, ne jediný „heavy chop".
