...
- **CHARACTER** — konkrétní COMPOSITES reprezentující humanoidní postavičku. Vizualizace: torzo (CylinderGeometry konická 0.19→0.15), hlava (SphereGeometry), 4 dvoudílné končetiny. Každá končetina je `Three.Group` s pivotem v rameni/kyčli; horní díl, kulový kloub uprostřed, a sub-Group pro spodní díl (pivot na kloubu). Atribut `COLOR` (oblečení/končetiny); hlava fixně pleťová v enginu. Default pose (bez `ANIMATE`) = loutka visící v klidu. Podporované `ANIMATE.kind`y: `walk`, `sit`, `lie`, `wander`. *(M8+, DD-18.)*
...
- **wander** — `ANIMATE.kind` pro CHARACTER: **stavový automat uvnitř animátoru** („poflakování se"). State v `group.userData.wander` (`current`, `timer`, `targetX/Z`, `subject`, `substate`). 6 stavů: `walk` (náhodný cíl v `bounds`), `run` (rychlejší + větší amp), `stand` (idle), `sit`, `lie`, `work` (approach k `subject` + perform sekání rukama nad hlavou). Transitions: timer 2–8 s, nebo po dosažení cíle (walk/run/approach), nebo při kolizi (DD-19 stop & transition). Parametry: `bounds` (poloviční šířka walkovací oblasti, default 3), `subjects` (pole instancí — např. `[rock1, tree1]` — pro random výběr ve `work` stavu). Sdílí pose helpers s `walk`/`sit`/`lie` animátory. *(DD-18.)*

## Kolize

- **collidable** — entita s definovaným kolizním radiusem v `collisionRadiusFor(instance)`. Radius 2D v rovině XZ (Y ignorován). Dispatch podle typu: CHARACTER 0.2, TREE 0.35, HOUSE 0.85, ROCK/CCUBES/TCUBES/plain CUBES 0.5; BALLOON/CLOUD/SPRITES/TIMER/COUNTER = `null` (neexistují v kolizní rovině). Viz DD-19.
