...
- **DECOR** *(sez. 39 kotva DD-49, bez impl)* — generická COMPOSITES třída pro krajinné dekorace (stromy/keře/kameny/tráva). Atributy: `KIND` (string lookup do `DECOR_BUILDERS`), `SEED` (number, deterministic varianty per instance), `SCALE` (default 1.0). Builder pattern: `buildSpruce/Oak/Bush/Rock/GrassTuft(group, { seed, scale })` mutuje prázdný `THREE.Group` faceted lowpoly primitivy (BoxGeom / CylinderGeom / ConeGeom / IcosahedronGeom(0) / SphereGeom low-segs) se sdíleným `MeshLambertMaterial({ flatShading: true })` per color. Shared toolkit (`src/composites/toolkit.js`): `lowpolyMat` per-color singleton + `getGeomCache` per-(KIND × part) geometry singleton + 6 paleta konstanty (`BARK_BROWN`/`LEAF_GREEN`/`LEAF_AUTUMN`/`ROCK_GRAY`/`GRASS_GREEN`/`BUSH_GREEN`) + `mulberry32` RNG. Biome-aware spawn: `DECOR_DENSITY[latitude][humidity]` lookup v `terrain.js`, `decorate` Krok 7 v `generateTerrain` → `decorations[]` pole. Y konvence DD-28 (mesh bottom = surface, spawn na `y = y_top + 1`). První-pass 5 KIND: `spruce`/`oak`/`bush`/`rock`/`grass_tuft`. Sub-prah: `stump`/`log`/`palm`/`cactus`/`flower`/sezónní varianty/`_snow` post-fix/density UI control/InstancedMesh refactor. *(Vrátí decoration vrstvu COMPOSITES po DD-32 wipe sez. 24 pixel-voxel TREE/GRASS_TUFT/ROCK_PIXEL/LOG.)*
...
- **WORLD** — singleton DO (Data Object) pro globální atributy scény. Bez `X/Y/Z` (DD-01 demonstrace separace vizuální/modelová entita). Aktuální atributy:
  ...
  - `DAY_SPEED ∈ ℝ` (cykly za sekundu; default `0` = paused).
