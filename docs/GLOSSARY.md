...
- **TCUBES** (texture cubes) *(plánováno)* — potomek CUBES s `TEXTURE_TOP`, `TEXTURE_BOTTOM`, `TEXTURE_NORTH`, `TEXTURE_SOUTH`, `TEXTURE_EAST`, `TEXTURE_WEST`. Per-face textury; nevyplněná plocha fallback šachovnice.
- **SPRITES** *(plánováno)* — potomek CUBES vizualizovaný jako 2D billboard (obrázek vždy otočený ke kameře). Použití: dialog bubble, label, 2D entita. Atribut `ASSET`.
...
- **M5+** — pozdější milníky: SPRITES (dialog bubble), TCUBES, chování v čase, nevizuální potomci OBJECTS.
