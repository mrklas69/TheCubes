...
## v1.1-voxel-mvp arc (sez. 50-53, fáze 1-2 close)

> **Arc start:** sez. 50 (2026-05-15) TODO triage T1-T8 + DD-56 voxel-native surovinový model immutable entry. **Arc cíl:** prezentace surovin a test manipulace s nimi. **Plán:** 4 sezení (render core → scatter weights → chop interakce → BALLOON transport).

- [x] **Sez. 50 — TODO triage + voxel arc start** *(docs-only)* — Fáze A: T1-T8 strukturní restructure TODO (83 → 71 ř., MERGE 4 sekcí Speculative subsekce, M-Genesis arc Fáze 1-5 → DONE). Fáze B: voxel surovinový model `%THINK` (16 Q&A → 14 klíčových konceptů DD-56). Commits `ac744a3` (T1-T8 triage), `e1a3322` (DD-56 + arc setup). User: *„Neprogramovat!"* — single arc-planning sezení bez implementace. Spec ready-to-execute pro sez. 51.
- [x] **Sez. 51 — voxel render core + DD-57 shuffle pivot + scatter** *(~510 ř., 5 patchi v jednom sezení)* — `src/resources.js` NEW (RESOURCE_REGISTRY 5 typů, Fisher-Yates shuffle), `model.js` VOXELS atribut + 4 helpery, `main.js` _voxelBatches + 4 dispatch helpery + slope normály + rubik init + scatter. **DD-57 pivot** (user „voxely namíchat"): drop DD-56 koncept 4 autosort render-side, insertion order Map drží data-side LIFO. Plus 5. surovina `dirt`, corner posazení, color kalibrace (Lambert + canonical hex), scatter `floor(X*Z/10)` s ramp tilt. F12 regrese `sizeX is not defined` (recovery scope-level deklarace prereq). Commit `61dc2cb`.
- [x] **Sez. 52 — per-biome scatter weights (Krok 8)** *(~100 ř.)* — `terrain.js` `BIOME_SCATTER_WEIGHTS` (12 buněk × 5 resources sum=1.0) + `scatterWeightsForBiome(latitude, humidity)` helper paralel `surfacesForBiome`. `main.js` `scatterRandomVoxels` 5. param `weights` + per-voxel weighted pick přes cumulative sum. Design os: LATITUDE → stone/sand/wood baseline, HUMIDITY → wood share. Acceptance pass napříč 4 climates single-pass clean. Commit `413e438`. Body 2-4 (RESOURCE_YIELD + chop + BFS) odsunuty na sez. 53.
- [x] **Sez. 53 — DECOR RESOURCE_YIELD + chop interakce + BFS overflow** *(~240 ř., v1.1 fáze 2 close)* — `src/composites/builders.js` (+~50 ř.) `RESOURCE_YIELD` lookup (10 KIND-ů, 7 chopable, **post-kalibrace 1 voxel napříč**: spruce/oak/palm/stump/log/cactus = 1 wood, rock = 1 stone; bush/flower/grass_tuft no-yield) + `hasChopYield(kind)` predicate. `src/main.js` (+~190 ř.) voxel infra extension: `_voxelOwners` Set, `_topTcubesByColumn` Map, `_occupiedCells` Set, `_chopBucketByColumn` Map, `_lastSpawnArgs` cache. `rebuildAllVoxels()` full re-dispatch (owners replay + scatter replay z cache). Chop dispatch: `getOrCreateChopBucket` (lazy abstract CUBES 1 cell nad top TCUBES = no z-fight) + `addYieldWithBFSOverflow` (4-neighbor max 3 hops + silent drop) + `chopDecor` + pointerdown listener. Hover diferenciace: `HOVER_EMISSIVE_CHOP = 0x802020` (warm red) pro chopable DECOR, žlutý jinde; cursor cue `pointer` nad chopable. **Design pivot mid-edit:** první návrh `addVoxel` přímo na TCUBES = z-fight risk → bucket pattern (rubik-like nad terrain top). **Post-commit kalibrace:** initial heavy yield 8/12/6/4/3/5/2 → 1 voxel napříč (= sběrný gameplay rhythm s multi-chop akumulací). User F12 smoke pass *„Test OK"*. Žádný nový DD (DD-49 spec extension + DD-56 koncept 11 implementace).
