...
- [x] **Sez. 53 — DECOR RESOURCE_YIELD + chop interakce + BFS overflow** *(~240 ř., v1.1 fáze 2 close)* — `src/composites/builders.js` (+~50 ř.) `RESOURCE_YIELD` lookup (10 KIND-ů, 7 chopable: spruce=8/oak=12/palm=6/stump=3/log=5/cactus=2 wood, rock=4 stone; bush/flower/grass_tuft no-yield) + `hasChopYield(kind)` predicate. `src/main.js` (+~190 ř.) voxel infra extension: `_voxelOwners` Set, `_topTcubesByColumn` Map, `_occupiedCells` Set, `_chopBucketByColumn` Map, `_lastSpawnArgs` cache. `rebuildAllVoxels()` full re-dispatch (owners replay + scatter replay z cache). Chop dispatch: `getOrCreateChopBucket` (lazy abstract CUBES 1 cell nad top TCUBES = no z-fight) + `addYieldWithBFSOverflow` (4-neighbor max 3 hops + silent drop) + `chopDecor` + pointerdown listener. Hover diferenciace: `HOVER_EMISSIVE_CHOP = 0x802020` (warm red) pro chopable DECOR, žlutý jinde; cursor cue `pointer` nad chopable. **Design pivot mid-edit:** první návrh `addVoxel` přímo na TCUBES = z-fight risk → bucket pattern (rubik-like nad terrain top). User F12 smoke pass *„Test OK"*. Žádný nový DD (DD-49 spec extension + DD-56 koncept 11 implementace).
