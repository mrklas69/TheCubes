...
## Rozšíření modelu
- **SPRITES** — potomek CUBES s 2D billboard vizualizací (dialog bubble, 2D postava, label). Atribut `ASSET`. → DONE (sez. 4, M5): třída + canvas-generovaná dialogová bublina + instance `dialog_0001` nad stromem. Dynamický 3D ocásek → DONE (sez. 8, DD-16). **→ DROPNUTO sez. 48 M-Genesis cleanup (DD-55)** — bez živého konzumenta po DD-32 terrain pivot, scope locked na 8 tříd. Revert z git history (sez. 4-8 commits) když přibude konzument (např. label nad postavou po Stickman integraci).
- **COMPOSITES** — potomek CUBES s 3D mesh z více částí. → DONE (sez. 3): základní třída. *(Sez. 15 DD-23 + sez. 24 DD-32: smazány TREE/BALLOON/HOUSE/CLOUD/ROCK/TUNNEL_ARCH/WAREHOUSE/TRAIN/GRASS_TUFT/ROCK_PIXEL/LOG/VOXEL_MODEL. CHARACTER/NOODLE/STICKMAN přesunuty sez. 14 do sibling `./source/Stickman`.)* Aktuální COMPOSITES potomci (sez. 33+40): **LAMP** (Victorian-style, SpotLight, DD-40), **DECOR** (DD-49+Fáze 6: spruce/oak/bush/rock/grass_tuft/palm/cactus/flower/stump/log + `_dead`/`_snowed`/`_autumn` modifiery). Pattern PATH (DD-27, vrstva 3 LINES) → DROPNUTO sez. 48 cleanup.
- **TCUBES** — potomek CUBES s per-face texturami. → DONE (sez. 4, M6): třída + dispatch `faceMaterialFor` (DD-14). DD-41 (sez. 34) atlas DD-36 nahradil lowpoly vertex-color. **Sez. 49 K1 cleanup** dropla i slow path `faceMaterialFor` + `:named-texture` paleta + `makeEmojiTexture` jako YAGNI residue (M6 milestone bez konzumenta po DD-32/DD-41/sez. 48 PATH drop). Fallback pro neznámý kind redukován na sdílený `_checkerboardMat` const inline. Hlavní use case: terrain bloky `InstancedMesh` batch path (DD-37).
...
- **Nevizualizované OBJECTS** — pravidla, recepty, timery, score. Dědí z OBJECTS přímo, ne z CUBES. → PARTIAL: **WORLD** singleton (DD-29 sez. 20 → DD-38 sez. 32 re-introduce + DD-42/50 climate/season). **→ TIMER + COUNTER DROPNUTÉ sez. 48 M-Genesis cleanup (DD-55)** — třídy + engine infrastruktura bez živého konzumenta po DD-32 terrain pivot. WORLD jediný aktivní nevizuální potomek OBJECTS. Revert TIMER + COUNTER + ACTIONS dispatch z git history (sez. 9 commit) když přibude konzument (např. gameplay vrstva discrete events nebo post-close FindPath/supply chain arc).
- **BUILDING** *(přesun z TODO sez. 50, 2026-05-15)* — budoucí dekorativní/factory entity nad generovaným terénem. Potomek COMPOSITES (paralel LAMP + DECOR pattern). Atributy TBD (`KIND` enum house/barn/tower/...? `SCALE`? `SNOWED`?). Procedurální builder per KIND v `src/composites/builders.js`. Relevant pro supply chain post-close arc (DD-56 kandidát = GENERATOR/TRANSFORMER/CONSUMER vizuální skin).
- **TRACK** *(přesun z TODO sez. 50, 2026-05-15)* — sourozenec PATH (dropnutý sez. 48). Vlaky odloženy. Pattern: 1. třída entity (= ne grafický overlay), POINTS sekvence (Catmull-Rom strip jako PATH), KIND enum (`rail`/`tram`/`mine`?). Revert PATH z gitu sez. 4-8 jako šablona až bude apetit. Relevant pro supply chain transport vrstvu.

**Zbývající:** RULE framework (condition + action), BRAIN (per-entity chování), pravidla/recepty s kombinačními ACTION — wait pro post-close gameplay arc (FindPath + generátor/transformátor/konzument).
