...
- **TCUBES** — potomek CUBES s per-face texturami. → DONE (sez. 4, M6): třída + dispatch `faceMaterialFor` (DD-14). DD-41 (sez. 34) atlas DD-36 nahradil lowpoly vertex-color (slow path `faceMaterialFor` zachován pro CCUBES default šachovnici + COMPOSITES boxy). Hlavní use case: terrain bloky `InstancedMesh` batch path (DD-37).
