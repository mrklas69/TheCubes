...
| `src/main.js` | Three.js scéna + builders dispatch (`createMeshFor`, `getTcubesKindGeom`, `getRampGeom`, `createPathFor`) + **lowpoly vertex-color pipeline (DD-41 nahrazuje DD-36 atlas)** + **InstancedMesh batches pro terrain (DD-37)** + post-process composer (fog + DOF/BokehPass) + sun mesh + **WORLD instance + `updateSun()` (LATITUDE-driven tilt DD-42)/`updateAtmosphere()` (DD-39)/`updateWorldTime(dt)`** + animators + hover + render loop + `buildScene`/`regenerateScene` (volá `generateTerrain` + `surfacesForBiome` G3 DD-44 + spawn) + `window.settings` toggle API |
| `src/terrain.js` | `generateTerrain({ size, relief, surfaces, seed })` — fBm/ridge³ heightmap (DD-45) + smoothstep bimodální pro r ≥ 6 (DD-46), biome map, sloupcové vyplnění, vodní plane(y), ramp smoothing layer (DD-33/34/35); exports `BIOME_NAMES` + `BIOME_SURFACES` + `surfacesForBiome` (DD-42/44), `maxReliefForSize` (G1) |
