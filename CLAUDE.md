...
**Factory toy test scéna (sez. 21 fáze A):** 3 fasility na grass podlaze (Y=0): `fac_forest_0` (-2,0,2) tmavě zelený, `fac_sawmill_0` (0,0,2) oranžový s pre-stocked 50 logs, `fac_storage_0` (2,0,2) béžový. Bez PATH (fáze B). HUD: 6 čítačů surovin + event log 5 řádků (PROD/DRN/PAUS/RSUM verbs).
...
OBJECTS (ID, NAME, DESCRIPTION, ANIMATE)
 ...
 │    ├── PATH (KIND, POINTS, SOURCE?, SINK?, THROUGHPUT?)   ← 1D křivka + Phase B transport (DD-27 + DD-31)
...
| Model | `src/main.js` (FACTORY TOY ENGINE) | `facilities[]` registry, `events[]` ring buffer, `productionTick(dt)` + `generatorTick`/`transformerTick` (material gate), `logEvent`/`renderEventLog` (floor-crossing PROD/DRN + state-change PAUS/RSUM), `aggregateResources()`, `createFacilityFor(instance)`. |
