...
## Project
TheCubes je **model-first 3D factory-observer toy s OOP modelem jako runtime** *(DD-30 pivot sez. 21 — 3D analogie sourozeneckého projektu Voidspan)*. Hráč staví fasility, suroviny tečou mezi nimi, model počítá throughput, hráč pozoruje ukazatele. Cíl = pozorování, ne win/loss. Sandbox/free-building aspekt zůstává (editor MVP plánovaný v Phase C). Historie: sez. 1–13 model-first sandbox s ad-hoc COMPOSITES, sez. 15 (DD-23) all-voxel pivot, sez. 16–17 pixel-voxel apartmá, **sez. 21 (DD-30) factory pivot**.

## Status
Milníky **M1–M7 hotové**, **M8+ průběžně**. Po sez. 21 (**DD-30 factory-observer pivot, DD-31 Resource model & FACILITY**) je projekt v aktivním pivotu na boční větvi `feat/factory`. **Sez. 22 fáze B:** `PATH` rozšířen o `SOURCE`/`SINK`/`RESOURCE`/`THROUGHPUT`, `pathTick(dt)` přesouvá items mezi buffery, TIME_SCALE slider v HUD, test scéna migrovaná na conveyor řetězec forest → sawmill → storage. Stará scéna (severská dioráma s 4-vrstvou taxonomií DD-25) zůstává jako pozadí beze změny do Phase D cleanup. Nová vrstva **FACILITY** (5. vrstva nad CUBES — sourozenec BLOCKS) přidává factory toy mechaniky: lokální buffery, recepty, material gate, event log. DD-26 sjednocená ORIENTATION, DD-27 PATH (rozšířeno DD-31 o transport), DD-28 sjednocená Y konvence, DD-29 WORLD singleton (rozšířeno DD-31 o `TIME_SCALE` + `RESOURCES` jako nové konzumenty).
...
5. **Fasility** *(DD-31 sez. 21)* — `FACILITY` rodina: GENERATOR (forest/quarry/well/coal_mine), TRANSFORMER (sawmill/crusher), STORAGE. Lokální buffery per fasilita, fixed-KIND transformery (izomorfie s TREE.KIND), placeholder mesh 1×1×1 box s plochou barvou per KIND. Data registries `RESOURCES_DEF` (6 surovin), `RECIPES_DEF` (2 recepty), `FACILITY_DEF` (7 KINDů).

**Factory toy test scéna (sez. 22 fáze B):** 3 fasility na grass podlaze (Y=0): `fac_forest_0` (-2,0,2) tmavě zelený, `fac_sawmill_0` (0,0,2) oranžový, `fac_storage_0` (2,0,2) béžový. **2 conveyor PATH** propojení: `path_forest_sawmill` (resource `logs`, throughput 2 ks/s) + `path_sawmill_storage` (resource `planks`, throughput 2 ks/s). HUD: 6 čítačů surovin + TIME_SCALE slider + event log 5 řádků (PROD/DRN/PAUS/RSUM verbs).
...
**Sez. 21:** **DD-30 pivot na 3D factory-observer toy** (Voidspan inspirace, ne dogma — selektivní převzetí Resource registry / Recipe matrix / Material gate / Event Log; odmítnuto 2D Belt topologie, kolonisté, Protocol, Coin, TS stack). Identita projektu se mění z „meta-sandbox" na **„model-first factory-observer toy s OOP modelem jako runtime"**. Cíl = pozorování ukazatelů (Voidspan-derived Perpetual Observer Simulation axiom), ne win/loss. **DD-31 Resource model & FACILITY** — 6 surovin (`logs/planks/stone/gravel/water/coal`), **lokální buffery** per fasilita (model B), **fixed-KIND transformery** (izomorfie s TREE.KIND — `sawmill`/`crusher` jsou KIND, ne data atribut), PATH `KIND ∈ "conveyor"|"pipeline"` s `SOURCE`/`SINK`/`THROUGHPUT` (Phase B). Tick: **1 wall s = 1 tick**, `productionTick(dt × world.TIME_SCALE)` v render loopu. **Soft revize DD-29:** `world.RESOURCES` agregát + `world.TIME_SCALE` jako noví konzumenti (`WIND_STRENGTH` zůstane platný, po Phase D cleanup ztratí `tree_sway` a migruje do IDEAS). **Boční větev `feat/factory` fáze A:** model.js +160/−15 (registry + FACILITY/GENERATOR/TRANSFORMER/STORAGE třídy + WORLD rozšíření), main.js +230/−5 (FACTORY TOY ENGINE: `productionTick`, `generatorTick`/`transformerTick` s material gate, `logEvent`/`renderEventLog` ring buffer 100/visible 5, floor-crossing PROD/DRN, state-change PAUS/RSUM, `aggregateResources`), index.html +50 (`#resources` 6 řádků + `#eventlog` severity-colored). Test scéna 3 fasility (forest/sawmill pre-stocked 50 logs/storage) bez PATH zatím. 4-fázový plán (A model+tick / B PATH transport / C editor MVP / D cleanup pixel-voxel apartmá). **Censure! (AI → AI):** identitní pivot měla AI nabídnout sama, ne čekat na user prompt po 4 dříve diskutovaných směrech uvnitř sandbox identity.
**Sez. 22:** **Fáze B PATH transport.** Body 2+3+5+6 z Příště. **`PATH` rozšířen** o `SOURCE`/`SINK`/`RESOURCE`/`THROUGHPUT` (default null = backward compat se sez. 17 dekorativní cestou); `RESOURCE` **explicit** (ne derived ze SOURCE — storage drží libovolné suroviny, derive by byl nejednoznačný). **`pathTick(dt)` + `transferOnPath`**: clamp `min(throughput*dt, source_have, sink_free)`, mutace bufferů, log DRN/PROD přes existující floor-crossing helpery (1-event-per-whole-unit granularita); volá se v render loopu **po** `productionTick` (pořadí důležité: tick = produkce, pak distribuce). **Boot-time validace category × KIND** v `createPathFor` (warn na `solid` / `pipeline` neshodu). **TIME_SCALE slider** v HUD (`#simctrl` divovec s `pointer-events: auto` override, range 0..3, `input` listener mutuje `world.TIME_SCALE` live). **Migrace test scény** — smazán `sawmill.BUFFER.logs = 50` pre-stock, 2 conveyor PATH (forest→sawmill, sawmill→storage, throughput 2 ks/s). Diff stat: model.js +24/−6, main.js +124/−8, index.html +26. Test OK. **Odhalený polish problem:** sawmill osciluje PAUS/RSUM kvůli source-limited supply (forest 0.5/s vs. recipe 1/s) — řešit sez. 23 (3 možnosti: druhý forest / ladění rates / `setPaused` debounce). **Bod 4** (material gate emissive) a **bod 7** (druhý řetězec quarry/crusher + well/coal_mine) odloženy na sez. 23. **Žádný nový DD** (sez. 22 implementuje DD-31 plán). **Censure! (AI → AI):** slider šel do `#hud` aniž by ukázala konkurenční umístění (bottom panel s #eventlog jako „observer dashboard" páreček) — drobnost, ale příště systematizovat při dalších UI elementech.
...
OBJECTS (ID, NAME, DESCRIPTION, ANIMATE)
 ...
 │    ├── PATH (KIND, POINTS, SOURCE?, SINK?, RESOURCE?, THROUGHPUT?)  ← 1D křivka + factory transport (DD-27 + DD-31 sez. 22)
 │    └── FACILITY (KIND, BUFFER, PAUSED, PAUSE_REASON)     ← factory toy (DD-31, sez. 21)
 │         ├── GENERATOR  (KIND ∈ forest/quarry/well/coal_mine) ← produkuje surovinu bez vstupu
 │         ├── TRANSFORMER (KIND ∈ sawmill/crusher)            ← recept dle RECIPES_DEF[KIND]
 │         └── STORAGE    (HOLDS)                              ← pasivní pufr sítě
 ├── TIMER (INTERVAL, ACTION)                              ← nevizuální (DD-17)
 ├── COUNTER (VALUE, INCREMENT)                            ← nevizuální, řádek v HUD
 └── WORLD (WIND_STRENGTH, TIME_SCALE, RESOURCES)          ← nevizuální singleton (DD-29 sez. 20 + DD-31 sez. 21)
```
...
| Model | `src/model.js` | `OBJECTS` (+`ANIMATE`), `CUBES`, `BLOCKS` (CCUBES/TCUBES/TRRAMPS/TTRAMPS/TTUNELS), `SPRITES`, `COMPOSITES` (TREE/GRASS_TUFT/ROCK_PIXEL/LOG/VOXEL_MODEL), `PATH`, `FACILITY` (GENERATOR/TRANSFORMER/STORAGE — DD-31), `TIMER`, `COUNTER`, `WORLD` (s TIME_SCALE+RESOURCES — DD-31). Base třídy `BLOCKS` + `COMPOSITES` mají explicit constructor s default `ORIENTATION = 0` (DD-26). Data registries `RESOURCES_DEF` / `RECIPES_DEF` / `FACILITY_DEF` (DD-31). |
| Model | `src/main.js` (FACTORY TOY ENGINE) | `facilities[]` + `paths[]` registry, `events[]` ring buffer, `productionTick(dt)` + `generatorTick`/`transformerTick` (material gate), `pathTick(dt)` + `transferOnPath` (factory transport, sez. 22), `logEvent`/`renderEventLog` (floor-crossing PROD/DRN + state-change PAUS/RSUM), `aggregateResources()`, `createFacilityFor(instance)`, TIME_SCALE slider listener. |
